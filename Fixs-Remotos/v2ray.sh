...
removeV2Ray() {
    #卸载V2ray脚本
    bash <(curl -L -s raw.github.com/lacasitamx/SCRIPT-LATAM/Netvps/Fixs-Remotos/go.sh) --remove >/dev/null 2>&1
    rm -rf /etc/v2ray >/dev/null 2>&1
    rm -rf /var/log/v2ray >/dev/null 2>&1

    #卸载Xray脚本
    bash <(curl -L -s raw.github.com/lacasitamx/SCRIPT-LATAM/Netvps/Fixs-Remotos/go.sh) --remove -x >/dev/null 2>&1
...
EOF
    ...
    #安装V2ray主程序
    [[ ${INSTALL_WAY} == 0 ]] && bash <(curl -L -s raw.github.com/lacasitamx/SCRIPT-LATAM/Netvps/Fixs-Remotos/go.sh)
}
